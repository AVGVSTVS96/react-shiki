...
/**
 * Attribution: AI generated tests
 */

/**
 * These tests are kinda redundant, we only really need to test that options make it to Shiki,
 * but these tests were easy enough to generate with AI so no harm in keeping them to be a little
 * more thorough.
 */
...
describe('Shiki Rendering Options', () => {
  ...
  describe('mergeWhitespaces option', () => {
    ...
    test('default behavior (true) - should merge whitespace tokens', async () => {
      const { getByTestId } = render(
        <TestHookComponent code={codeWithSpaces} language="javascript" />
      );
  ...
  describe('structure option', () => {
    ...
    test('default classic structure', async () => {
      const { getByTestId } = render(
        <TestHookComponent code={sampleCode} language="javascript" />
      );
  ...
  describe('decorations option', () => {
    ...
    test('decorations on second line (as in confirmed example)', async () => {
      ...
      await waitFor(() => {
        ...
        const decoratedElement = container.querySelector(
    ...
    test('decorations with custom transform function', async () => {
      ...
      await waitFor(() => {
        ...
        expect(decoratedElement).toBeInTheDocument();
        expect(decoratedElement.getAttribute('data-transformed')).toBe(
          'true'
        );
      });
    });
